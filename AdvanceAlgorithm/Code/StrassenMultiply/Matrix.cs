...
namespace StrassenMultiply
{
    public sealed class Matrix
    {
        ...
        public int Rows
        {
            ...
            set
            {
                var temp = Array.CreateInstance(values.GetType().GetElementType(), new int[] { value, values.GetLength(1) });
        ...
        public int Columns
        {
            ...
            set
            {
                var temp = Array.CreateInstance(values.GetType().GetElementType(), new int[] { values.GetLength(0), value });
                int length = values.Length <= temp.Length ? values.Length : temp.Length;
                Array.ConstrainedCopy(values, 0, temp, 0, length);
                values = (int[,])temp;
            }
